<template>
    <div
        class="d-flex flex-column pt-4 justify-content-start m-0 container-fluid normal-font"
    >
        <a
            class="btn-show border border-2 position-absolute top-0 start-0 m-3 z-3 animate__animated animate__zoomIn"
        ...
        <div class="mb-5">
            <h1
                class="text-center mb-4 fw-bold text-capitalize animate__animated animate__zoomIn"
            >
                {{
                    new Date(newDate[0].data).toLocaleDateString("it-IT", {
                        weekday: "long",
                        year: "numeric",
                        month: "long",
                        day: "numeric",
            ...
            >
                <div
                    class="row align-items-center justify-content-center w-100 m-0"
                >
                    <!-- CREATE -->
                    <div
                    ...
                    >
                        <div class="mb-3 w-100">
                            <label class="mb-1 fs-4" for="titolo"
                                >Titolo:
                            </label>
                            <div
                        ...
                        <div class="mb-3 w-100">
                            ...
                            >
                                <select
                                    id="mealType"
                                    class="form-control"
                                    v-model.trim="data.show_type_id"
                                    @change="
                                        getArtistsByShowType(data.show_type_id)
                                    "
                                >
                                    <option
                                        v-for="(show, x) in showTypes"
                                        :key="x"
                                        :value="show.id"
                                        :selected="
                                            show.id === data.show_type_id
                                        "
                                    >
                                        {{ show.nome }}
                                    </option>
                            ...
                            >
                                ...
                                >
                                    <input
                                        ...
                                        @change="
                                            addArtistsInFormdata(artist, data)
                                        "
                                        :checked="
                                            data.artists.some(
                    ...
                    >
                        <label class="mb-1 fs-4" for="descrizione"
                            >Descrizione:
                        </label>
                        <!-- <textarea
                            id="descrizione"
                            class="form-control mb-2"
                            v-model.trim="data.descrizione"
                        ></textarea> -->
                        <QuillEditor
                            theme="snow"
                            class="w-100"
                            v-model:content="data.descrizione"
                            contentType="html"
                        />
                        <div class="d-flex flex-wrap mb-3">
                            <div
                                ...
                                :key="index"
                                class="w-50 position-relative date-image-container p-2"
                            >
                                <img
                                    :src="image.path"
                                    alt="Anteprima immagine"
                                    class="w-100 h-100 date-image-preview"
                                />
                                <form
                                    @submit="deleteImage(image.id)"
                                    enctype="multipart/form-data"
                                >
                                    <button
                                        class="btn btn-danger delete-image fs-2"
                                    >
                                        <i
                        ...
                        <h3 class="normal-font">
                            Anteprima Immagini
                            <span class="fs-6"
                                >( le immagini saranno aggiunte in coda se prima
                                non elimini quella/e attuale/i)</span
                            >
                        </h3>
                        <div
                            v-if="data.imagePreviews.length > 0"
                            class="d-flex flex-wrap mb-3"
                        >
                            <div
                                v-for="(image, index) in data.imagePreviews"
                                :key="index"
                                class="w-50 p-2 position-relative date-image-container"
                            >
                                <img
                                    :src="image"
                                    alt="Anteprima immagine"
                                    class="me-2 w-100 h-100 date-image"
                                />
                                <button
                                    class="btn btn-danger delete-image fs-2"
                                    @click="removeImageFromPreview(data, image)"
                                >
                    ...
                    <div
                        class="col-12 d-flex align-items-center justify-content-evenly position-relative"
                    >
                        <!-- DELETE -->
                        <button
                            type="button"
                            class="btn-show bg-danger border-danger text-white normal-font py-3 text-uppercase rounded-5 px-4 fs-5"
                            data-bs-toggle="modal"
                        ...
                        <!-- EDIT -->
                        <button
                            class="btn-show border-warning normal-font border-2 text-center bg-warning"
                            type="submit"
        ...
        <div v-if="newDate.length === 1" class="container-fluid">
            <div class="row">
                <div class="col-12 text-center">
                    <a
                        :href="
                            route('dashboard.date.create', {
                                data: newDate[0].data,
                            })
                        "
                        class="btn-show border border-2 animated animate__zoomIn"
                        >Vuoi Aggiumngere un altro spettacolo ?</a
                    >
                </div>
            </div>
        </div>
    ...
    <!-- DELETE Modal -->
    <div
        v-for="(data, x) in newDate"
        :key="x"
        class="modal fade"
        :id="'exampleModal_' + data.id"
        tabindex="-1"
        :aria-labelledby="'exampleModalLabel_' + data.id"
        aria-hidden="true"
    >
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                >
                    <div></div>
                    <h1
                        class="modal-title fs-5 text-danger text-uppercase normal-font"
                        :id="'exampleModalLabel_' + data.id"
                    >
                        <i class="fa-solid fa-exclamation"></i> Attenzione
                ...
                <div class="modal-body text-center">
                    <p class="fs-3">
                        Sei sicuro di voler eliminare lo spettacolo di
                    ...
                    <div
                        class="border-0 d-flex w-00 align-items-center justify-content-evenly"
                    >
                        <button
                            type="button"
                            class="btn btn-secondary normal-font"
                            data-bs-dismiss="modal"
                        >
                            Annulla
                        </button>
                        <button
                            type="button"
                            class="btn btn-danger fw-bold normal-font"
        ...
        async function updateReservationDate(data) {
            ...
            try {
                const confirmEdit = window.confirm(
                    "sicuro di voler apportare le modifiche ?"
                );
                if (confirmEdit) {
                    axios
                        .post("/api/update-reservation-dates/", formData)
                        .then((res) => {
        ...
        async function deleteImage(id) {
            try {
                const confirmDelete = window.confirm(
                    "Eliminando le foto la pagina verra ricaricata e perderai le modifiche"
                );
                if (confirmDelete) {
                    axios
                        .delete("/api/delete-img-reservation/" + id)
                        .then((res) => {
